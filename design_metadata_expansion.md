...
| **6** | 残りサイトのメタ取得＋青空文庫の図書カード書誌＋訳者の contributor 分離 | **実装済み**（§11） |
...
## 11. Phase 6 実装結果（2026-07-26）— novel_downloader 側の完了

**全17サイトでメタデータを取得するようになった。** 取得元は CLAUDE.md の 6d を参照。

### 11.1 スコープ限定が繰り返し必要だった

Phase 3 のアルファポリスと同じ罠が複数サイトで再現した。作品ページに推薦カードが並ぶサイトでは、
ページ全体を検索すると**別作品の値を拾う**。実測した出現回数は次のとおり。

| サイト | 要素 | ページ内の出現回数 | 作品本体のスコープ |
|---|---|---|---|
| アルファポリス | `文字数` | 9回（最初の1件が推薦カード） | `div.p-sidebar-content-info__detail` |
| アルファポリス | `c-attribute-tag` | 57回 | `div.p-content-info` |
| berry's cafe | `.genre_name` | 60件以上 | `div.subDetails-01` |
| エブリスタ | `-finished` | 11回 | `div.novelDataWrap` |

### 11.2 ジャンルを決め打ちしないこと

実装中、翻訳2サイトにジャンルを固定（杉田玄白＝評論、結城浩＝文芸）したところ、
**『鏡の国のアリス』が「評論・エッセイ」に分類された**。両サイトとも思想・経済の論考と文学作品の
翻訳が混在するため固定できない。ソリスピアの `.genre`（「ライトノベル」）も媒体区分であって
内容ジャンルではないので使わない。**判定できないときはジャンルを付けない**という原則を守る。

### 11.3 訳者の contributor 分離（しおり互換に影響）

杉田玄白・結城浩は `dc:creator` を「原著者（訳者 訳）」から**原著者のみ**に変え、訳者を
`dc:contributor role="trl"` に移した。yomikake のしおりキーは書名＋`dc:creator` なので、
**この2サイトの既存 ePub のしおりは無効になる**（設計 §8 で想定済み・リリースノートに明記する）。
青空文庫の入力者・校正者も `trc` / `pfr` として contributor に入る。

### 11.4 その他の判断

- 青空文庫の底本書名は**作品名と同じときはシリーズにしない**（「こころ」→ 情報量なし。
  「太宰治全集3」→ シリーズとして有用）。
- `_iso_date` が青空文庫の「1988（昭和63）年10月25日」形式を扱えるよう元号括弧を除去する。
- ネオページはパンくずより**埋め込みの作品オブジェクト**が確実（分類・完結・文字数・話数・日時が揃う）。

### 11.5 検証

- `novel_health_check.py` **17/17 成功**
- epubcheck 5.3.0 で **0エラー / 0警告**（青空文庫の contributor 付き・杉田玄白の訳者分離を含む）
- 青空文庫の `.txt` → ePub → `.txt` 往復でメタデータが完全一致

### 11.6 novel_downloader 側で残っている項目

- ハーメルン・ステキブンゲイ・ソリスピアはバッジが JS で後付けされるため取得が限定的
- 野いちご／ノベマ！／berry's cafe は話数（章数）を meta に入れていない（`--start/--end` の単位が章のため）
