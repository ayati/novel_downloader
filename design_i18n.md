...
> 数値・行番号・関数名は実測時点のもの。再現は `tools/extract_messages.py`。
>
> **進捗（v2.10.0）**: Step B・Step 1・Step 2a は**実装完了**。
> `T()` / `_UI_LANG` / `_MESSAGES_EN`（96 件）が入り、P0 の 129 箇所すべてが `T()` 経由になった。
> 検査は `tools/check_i18n.py`。**残るは Step 0（定型出力の集約）と Step P（`--progress-json`）**で、
> どちらも i18n ではなく重複排除と GUI 連携の課題（§3.0 の依存関係も参照）。
...
**Step 0 / Step P は i18n の完了に必要ではなかった。** §2.1 の 8 ブロックはすべて P2/P3 で、
実測の結果 P0 とは **1 箇所も重ならない**（`tools/extract_messages.py` で確認）。
この 2 つは重複排除と GUI 連携プロトコルの課題として独立に進める。

> 実装時の実測: msgid は 95 ではなく **96**（`[警告] Pillow での JPEG 表紙生成中に…` は
> 文字列連結を含むため自動変換できず手動対応した分が 1 件増えた）。
