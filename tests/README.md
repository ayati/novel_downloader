...
- **合成イベントと実イベントを混ぜない。** `pump()` は Tk のイベントループを
  回すので、その間にウィンドウマネージャから本物の `<FocusIn>` /
  `<FocusOut>` が飛んでくる。テストが仕込んだ `_focused` の値が上書きされ、
  **実行するたびに結果が変わる**（実際に踏んだ）。窓の状態に依存する値は
  **確かめる直前に入れ、間に `pump()` を挟まない**
