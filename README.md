...
スマホの共有メニューからテキストファイルとして OneDrive / Google ドライブへ保存しておくと、
GUI がそのフォルダを見に行き、作品を一覧にします。

- ファイル本文から URL を全件抽出します（短縮 URL も展開して作品名・話数を先読みします）
- 取得する前に**タイトル・著者・全話数が見える**ので、何の作品か思い出してから決められます
- すでに本棚にある作品は「取得済み」と表示され、二重にダウンロードしません
- 取り込みは起動時・一定間隔（既定 5 分・0 で無効）・ウィンドウに戻ったとき・手動ボタンの 4 つの契機で行います
- ダウンロードに成功したファイルは `done\` へ移動します（`forwindows/watcher.bat` と同じ規約）
...
- 「新着チェック」で全作品の新着話数をまとめて確認できます
- 新着のある作品だけを選んで、あるいはまとめて「続きを取得」できます
