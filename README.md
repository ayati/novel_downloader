...
サイト負荷軽減のため、並行接続を避け、アクセスに1秒以上の間隔を空ける対処をしています。また生成するePubには元サイトへのリンクをトップページと奥付に設置し、サイトへの導線としています。
...
- 短縮 URL を自動展開してから処理（share.google / search.app / bit.ly / t.co / lin.ee / amzn.to 等 30 種以上対応）
  - リダイレクト先はホップごとに検査し、`http(s)` 以外とプライベート・ループバック・リンクローカルへの転送は追わない（v2.14.0）
- 上記各サイトの全話を自動取得（URL から自動判別）
  - 小説家になろうの**短編**（全 1 話・目次ページを持たない作品）にも対応（v2.14.0）
- 青空文庫書式テキスト（.txt）出力
  - ルビ記法（`漢字《かんじ》`）を保持
  - ヘッダーにあらすじ・底本 URL を記載
- **青空文庫の外字注記を Unicode 文字に自動変換**
  - `※［＃「にんべん＋參」、第4水準2-1-79］` のような外字注記を自動的に Unicode 文字に置換（例: 山月記の主要登場人物「袁傪」、放浪記の「涶」など）
  - 対応形式: `U+XXXX`（Unicode 直接指定）／`第3水準1-X-Y`・`第4水準2-X-Y`（プレフィックス付き JIS X 0213）／プレフィックスなしの素の `P-R-C` 表記／`U+XXXX、ページ-行` 併記形／複合説明（説明部に「、」を含む）
  - JIS X 0213 マッピング表は `data/aozora_gaiji_jis0213.tsv`（11,233 字、Project X0213 由来）を同梱
  - Unicode 未収録の真の外字（ドグラ・マグラの `感嘆符三つ` など）は `※（説明）` 形式へ自動フォールバック
  - ZIP 由来テキストの冒頭に `底本URL：`、末尾に `図書カード：` を自動挿入（v1.8.1）。他サイトと同様に `--append` / `--check-update` で更新監視が可能
- 縦書き ePub3（.epub）出力（デフォルト）/ 横書き ePub3（`--horizontal`）出力
  ...
  - macOS では標準のヒラギノ明朝を自動で使う（v2.17.0。以前は必ず SVG 表紙になっていた）
...
- 差分ダウンロード・追記機能（`--append`）：既存 `.txt` に続きを追記し ePub を再生成。新規エピソードがない場合はファイルを上書きしない。`--notify webhook` で Discord / Slack への Webhook 通知も可能
- ディレクトリ一括追記（`--append-dir`）：フォルダ内の全 `.txt` を走査し、新着のある作品だけ差分ダウンロード・追記・ePub 再生成。事前に対象一覧を表示して確認を求める（`--yes` でスキップ可）。`--notify webhook` で Discord / Slack への Webhook 通知も可能
- 更新確認機能（`--check-update`）：ダウンロード・上書きなしで新着話数・タイトルのみ表示。`--notify webhook` で Discord / Slack への Webhook 通知も可能
- ディレクトリ一括更新確認（`--check-update-dir`）：フォルダ内の全 `.txt` を走査し、各作品の新着有無をまとめて表示。`--notify webhook` で Discord / Slack への Webhook 通知も可能
...
- サイト公式サムネイルを表紙として使用（`--use-site-cover`）
  - 表紙にならない画像（横長のシェア用カード・サイト共通のプレースホルダ・極小サムネイル）は自動判定して除外し、自動生成表紙にフォールバック（v2.5.0）
  - エブリスタは公式の表紙画像、berry's cafe・ノベマ！は 2 倍解像度版を優先取得（v2.5.0）
- 本文中の挿絵を ePub に埋め込み
  - エブリスタ（v2.5.0）
...
受信箱を見に行く間隔／終わったときの知らせ（タスクバーを光らせる・音を鳴らす）。

右上の **日本語 / English** はヘルプやエラー表示の言語を切り替えます（作品本文は変わりません）。

### 画面の構成

上から URL 欄・大きなボタン・進み具合、その下に「▸」で開く 4 つの欄が並びます。
開いた状態は次回の起動でも覚えています。

| 欄 | 内容 |
|---|---|
| 📄 詳細ログ | エンジンの出力そのまま。失敗の原因を追うとき |
| 📥 受信箱 | 出先でスマホから投げた作品の受け取り口 |
| 📚 本棚 | ダウンロード済み作品の一覧と更新管理 |
| 詳細設定 | 保存先・表紙・動作・通知など |

複数の URL は「＋」で一覧に積んでからまとめて処理できます。1 件失敗しても
残りは続行し、終了後に失敗分だけ再試行できます。

### 受信箱（v2.14.0）

外出先のスマホで見つけた作品を、**共有フォルダ経由で PC に投げておく**ための機能です。
スマホの共有メニューからテキストファイルとして OneDrive / Google ドライブへ
保存しておくと、GUI がそのフォルダを見に行き、作品を一覧にします。

- ファイル本文から URL を全件抽出します（短縮 URL も展開して作品名・話数を先読み）
- 取得する前に**タイトル・著者・全話数が見える**ので、何の作品か思い出してから決められます
- すでに本棚にある作品は「取得済み」と表示され、二重にダウンロードしません
- 取り込みは起動時・一定間隔（既定 5 分・0 で無効）・ウィンドウに戻ったとき・手動の 4 契機
- 未取得の作品は `☰` で**話の一覧**を表示し、**行を選んで「ここから取得」**できます
  （サイトで途中まで読んだ作品の続きだけを落とす用途。`--start` 相当）
- `🔗 元サイトを開く` でブラウザに作品ページを開けます
- ダウンロードに成功したファイルは `done\` へ移動します（`watcher.bat` と同じ規約）
- URL の無いファイルは削除も移動もせず、印を付けて残します

### 本棚（v2.14.0）

出力先フォルダにある `.txt` を一覧し、更新の確認と続きの取得をまとめて行えます。
専用のデータベースは持たず、`.txt` ヘッダーの `底本URL：` を索引として使います。

- 並び順は「更新が新しい順（既定）／名前順／話数が多い順」。
  **この並び順がそのまま新着チェックの確認順**になります
- 「🔄 新着チェック」は**調べ終わった作品から順に行の表示が変わり**、
  いま調べている作品が分かり、**途中で中止**できます（そこまでの結果は残ります）
- 新着のある作品だけ、あるいはまとめて「続きを取得」できます
- `☰` は手元にある話の一覧（**通信しません**）、`📖` はその作品の ePub を開きます
- 話数にマウスを乗せると、最後に取れている話の題が出ます
- 受信箱の「ここから取得」で**途中から落としたファイル**は「部分（N番目から）」と表示され、
  先頭が欠けているため「続きを取得」は使えません（追記すると順序が壊れるため）
...
| `--append FILE` | — | 既存 `.txt` を指定して続きを追記・ePub 再生成。ヘッダーの `底本URL：` から URL を自動検出し差分ダウンロードを実行する。URL 指定不要。新規エピソードがない場合は既存ファイルを上書きしない。`--notify webhook` 対応 |
| `--append-dir DIR` | — | ディレクトリ内の全 `.txt` を走査し、新着エピソードがある作品だけ差分ダウンロード・追記・ePub 再生成する。事前チェック → 確認プロンプト → ダウンロード → サマリー表示。`--yes` で確認スキップ。`--notify webhook` 対応 |
| `--yes` | — | `--append-dir` の確認プロンプトをスキップする（自動化用） |
| `--check-update FILE` | — | 既存 `.txt` を渡してサイトの最新話数と比較し、新着話数・タイトルを表示して終了。ダウンロード・ファイル上書きは一切行わない。`--append` 前の確認に使う。`--notify webhook` と組み合わせて新着のみ Webhook 通知可能 |
| `--check-update-dir DIR` | — | ディレクトリ内の全 `.txt` を走査し、各作品の新着エピソードを一括確認する。底本URL のないファイルは自動スキップ。ダウンロード・ファイル上書きは一切行わない。`--notify webhook` と組み合わせて新着のみ Webhook 通知可能 |
| `--check-update-order {name,updated,episodes}` | `name` | `--check-update-dir` が作品を確認する順番（ファイル名順／更新日の新しい順／手元の話数が多い順）。作品数が多いと全件の確認に時間がかかるため、新着の出やすいものから先に確認したいときに使う |
| `--list-only` | — | ダウンロードせずエピソード一覧と話数のみ表示して終了する |
| `--dry-run` | — | 作品情報（タイトル・著者・総話数）を取得して表示したあと終了する。ダウンロード・ファイル出力は一切行わない。`--list-only` より軽い接続確認用 |
| `--lang {ja,en}`, `-L` | `ja` | ヘルプ等の表示言語。`--lang` / `-L` → 環境変数 `NOVEL_DOWNLOADER_LANG` → `ja` の順で決まる。`LANG` / `LC_ALL` は見ない。作品本文は変わらない |
| `--start N` | `1` | 取得開始位置。**刻む単位はサイトによって違います**（下表） |
| `--end N` | 最終話 | 取得終了位置。単位は `--start` と同じ |
...
| `--no-inline-images` | — | 本文中の挿絵を取得せず ePub にも埋め込まない。表紙画像には影響しない。サイト側の仕様変更で挿絵の取得に失敗するようになった場合の回避に使う |
...
| `--use-site-cover` | — | 作品ページの公式サムネイル画像を表紙として使用する。`--cover-image` が指定されている場合は `--cover-image` が優先。表紙にならない画像（横長・正方形／幅 200px 未満／サイト共通のプレースホルダ）は自動で除外し、自動生成表紙にフォールバックする |
| `--font FILE` | — | ePub 本文に埋め込むフォントファイル（.otf / .ttf / .woff / .woff2）。ファイルが存在しない場合は警告を出して埋め込みなしで続行 |
| `--cover-font FILE` | 自動検出 | 自動生成する表紙画像の題名・著者名を描くフォントファイル（.ttf / .otf / .ttc）。本文のフォント（`--font`）には影響しない。存在しない・日本語の字形を持たないファイルの場合は警告を出して自動検出のフォントで続行 |
...
| `--notify {stdout,webhook}` | `stdout` | 通知方法。`stdout`: 標準出力（新着なし・エラーなし時は無音）。`webhook`: `--webhook-url` 宛に POST。`--watch` / `--append` / `--append-dir` / `--check-update` / `--check-update-dir` で有効 |
...
サイトによって「話」「章」「ページ」と異なり、一部のサイトは対応していません。
...
非対応の 3 サイトは 1 作品 1 ファイルや章立ての翻訳文書で、話数中心の構造ではないためです。
指定してもエラーにはならず、全件を取得します。

### GUI 連携用の読み取り専用モード

GUI（`novel_downloader_gui.py`）がエンジンを呼び出すために使うモードです。通常の利用では必要ありません。
いずれも結果を JSON で標準出力に書いて終了し、ファイルの書き換えは行いません。

| オプション | 説明 |
|---|---|
| `--detect-site URL` | URL のサイト種別を判定して JSON 1 行を出力する。オフライン・即時。短縮 URL は展開しないため `site` は `null` になるが、`short_url` が `true` になる |
| `--list-sites` | 対応サイトの一覧を JSON で出力する |
| `--shelf-scan DIR` | ディレクトリ内の `.txt` を走査し、題名・著者・底本 URL・**手元の話数**・同名 ePub の有無などを JSON 配列で出力する（v2.14.0）。オフライン |
| `--progress-json` | 進捗を JSON Lines で標準出力に出し、人間向けの表示は標準エラー出力へ回す。指定しなければ従来どおりの出力 |
...
1. `--cover-bg` で明示指定した色
2. **作品のテーマカラー**（作者がサイト上で選んだイメージカラー。現在カクヨムのみが対応し、54 色から選択される）
3. 下表のサイト別デフォルト色
4. `#16234b`（不明なサイト）

いずれの場合も、タイトル文字が読めるように明度だけを自動調整します（色相・彩度は作者が選んだまま保たれます）。テーマカラーは `.txt` のヘッダーにも記録されるため、`--from-file` / `--append` で作り直しても同じ色が再現されます。
...
# 新着確認 + Discord Webhook 通知
python novel_downloader.py --check-update 作品タイトル.txt \
    --notify webhook --webhook-url https://discord.com/api/webhooks/xxx/yyy

# 続きを追記して ePub を再生成（URL 指定不要）
python novel_downloader.py --append 作品タイトル.txt

# 追記 + Discord Webhook 通知
python novel_downloader.py --append 作品タイトル.txt \
    --notify webhook --webhook-url https://discord.com/api/webhooks/xxx/yyy
...
# 一括追記を確認プロンプトなしで実行 + Discord Webhook 通知
