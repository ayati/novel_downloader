...
| `--start N` | `1` | 取得開始位置。**刻む単位はサイトによって違う**（下表） |
| `--end N` | 最終話 | 取得終了位置。単位は `--start` と同じ |

**`--start` / `--end` の単位と対応状況**（実測・`getattr(args, "start", ...)` の形で受けているサイトもあるので、grep で `args.start` だけを探すと取りこぼす）:
...
- **ノベマ！は「話」**（`ep_ranges[start_ep - 1:end_ep]`）。以前この表は「章番号」と書いていたが誤り
- 非対応の 3 サイトは**話数中心の構造ではない**（1 作品 1 ファイル・章立ての翻訳文書）ため、範囲指定の意味が薄い。指定してもエラーにはならず全件取得になる
- 野いちご・berry's cafe・ノベマ！・エブリスタは `_dry_run_exit` の `unit` が `"page"` で、**画面に出る総数（ページ数）と `--start` の単位（章・話）が一致しない**点に注意
...
| `--detect-site URL` | — | GUI 用の読み取り専用モード。URL のサイト種別を判定し JSON 1行（`{"schema":1,"site","display_name","needs_playwright","normalized_url","short_url","start_from_list"}`）を stdout に出力して終了。オフライン・即時・**短縮URL展開なし**。未対応サイトは `site:null`、ハーメルンは `needs_playwright:true`。`detect_site()`/`normalize_url()`/`_SITE_DISPATCH` を流用。**`short_url` は `is_short_url()`（`_SHORT_URL_HOSTS` のホスト名照合のみ・オフライン）の結果**で、`share.google` 等は `site:null` かつ `short_url:true` になる。**呼び出し側は `site is None` だけで「未対応」と判断してはいけない** — 展開すれば落とせる URL をエンジン起動前に捨てることになる（design_gui_v2.md §8.8）。**`start_from_list` は「話の一覧の行番号をそのまま `--start` に渡してよいか」**（`start_from_list_ok()`）で、`--start` に対応しているかとは**別の概念**。一覧に出している単位と `--start` が刻む単位が一致しているかを見る（エブリスタは一覧が話・`--start` がページなので false、杉田玄白・結城浩・青空文庫は `--start` 非対応で false・design_gui_v2.md §8.19） |
...
| `--progress-json` | — | GUI 連携用。**stdout を JSON Lines のイベント専用にし、人間向け出力は stderr へ回す**（`_main()` 冒頭で `sys.stdout` を差し替えるので既存の `print()` は無改修）。イベントは `stage` / `progress` / `output` / `workinfo` / `checkresult` / `episodes` / `checkstart` の7種。**指定しなければ挙動は一切変わらない**。仕様は `design_progress_json.md` |
...
2. **ePub3 ビルダー** — stdlib の `zipfile` で ZIP を直接生成。DPFJガイド v1.1.4 準拠で `<html class="vrtl">` / `<html class="hltr">` による組み方向制御を採用。主要関数：
   ...
   - `_draw_vtext` / `_draw_vtext_auto` — 縦組み描画。raqm があれば `draw.text(..., direction="ttb")` がフォントの vert 機能で `「」` の回転・長音符の縦字形を当てる。**raqm が無い環境（Android の Chaquopy・PyInstaller の exe・fribidi DLL の無い Windows）では `direction` を渡すと例外になり、以前は表紙ごと SVG に落ちていた**ため、`_has_raqm()` が偽なら `_draw_vchar_basic()` で 1 字ずつ組む（`_VERT_FORMS`＝Unicode の縦書き用互換文字 `﹁︵︙︑` 等への置換／`_VERT_ROTATE`＝互換文字の無い `ー～＝` 等を 90 度回転／小書き仮名を右上へ 0.1em 寄せる）。**互換文字を持たないフォントもある**（同梱の AyatiShowaSerif は vert 機能で縦の字形を持つのに U+FE10〜FE4F を1字も割り当てていない。Android の表紙はこのフォント）ので、`_font_has_glyph()` で .notdef と比べて無ければ、括弧・リーダー類は元の字を倒し（`【`→`︻` と同じ形）、`、。` は `_VERT_SHIFT` としてインクの中心を字枠中心について点対称に移す（`getbbox` は送り幅込みの箱を返すので実際に描いて測る）。字送りは両経路とも 1em で、raqm 版との差は数 px。テストは `tests/test_engine_offline.py` が raqm を強制的に無効にして確かめる。`_draw_vtext_auto` は CJK を含まない文字列（`@tea_kaijyu` のような欧文筆名・`berry's cafe` 等のサイト名）を**時計回り 90 度に倒して**描く（縦組みにおける欧文の作法。1 文字ずつ積むと読めない）
...
- **サイト表示名（小説家になろう・カクヨム等）は固有名詞なので訳さない。** 訳すと検索も `_SITE_COLOR_BY_LABEL` の照合もできなくなる

### 対応しない層1：出力データの構造（訳すと壊れる）

`.txt` 内の日本語ラベルの一部は**表示ではなくパースキー**であり、**青空文庫書式は UI ではなくデータフォーマットの仕様**として扱う。**これらは日本語で固定する。**

| ラベル | 読む側 | 訳した場合 |
|---|---|---|
| `底本URL：` | `_extract_url_from_txt` | `--append` / `--check-update` / `--append-dir` / `--watch` が全滅 |
| `配信元：` `ジャンル：` `状態：` 等（`_META_FIELDS` の12ラベル） | `_parse_meta_lines` | メタ情報が round-trip で消える |
| `【あらすじ】` | ヘッダー境界の判定 | あらすじがラベル行を巻き込む |
| `［＃「…」は大見出し］` | `_body_lines_to_xhtml` | 見出しが本文化する |

さらに**これらは出力済みの `.txt` にも埋め込まれている**ため、englishization はユーザーの手元にある過去の全ファイルを `--append` 不能にする。奥付の `底本：` / `入力：` / `校正：` / `作成：` と日付書式 `%Y年%m月%d日` も同様に固定。

### 対応しない層2：組版処理（英語には原理的に合わない）

縦組み日本語組版が本ツールの強みであり、英語対応のために薄めない。**英語小説の対応が必要になったら別ツールとして設計する。**

- `normalize_tate()` は全サイトの本文全体に無条件適用され、英語を通すと破壊する（実測: `Wait! What?! No...` → `Wait！ What？！ No……` / `Hello!World` → `Hello！　World`）。**言語判定でスキップする改修はしない**
- `_draw_vtext_auto` は非 CJK 文字列を時計回り90度に倒して描く（縦組みにおける欧文の作法として正しい）
- 縦中横・ルビ・`_char_class` はいずれも日本語の文字種前提
- `_find_cjk_fonts` は CJK フォントを要求し、無ければ SVG 表紙にフォールバックする（既定の挙動として許容）
...
編集したら**まずこれを走らせる**。外部ライブラリは使っていない。

```bash
python3 tests/run_all.py                        # オフラインのみ（十数秒）
NOVEL_TEST_NETWORK=1 python3 tests/run_all.py   # 実サイトに当てるものも含める
```

画面が要るスイート（GUI）は `DISPLAY` が無い環境では自動でスキップする。
書くときの約束・拾えない範囲は `tests/README.md`。**UI は「ウィジェットを
操作して画面がどうなるか」で書くこと** — 関数の戻り値を見る書き方では、
実機で踏んだ不具合を取り逃した実績がある（design_gui_v2.md §8.13a）。

### 手動での確認

テストで拾えない範囲（実機の見た目・サイト構造の変化・ePub の仕様適合）は手動で見る：
...
- **Android のビルド環境は AGP 9.2.1 / Gradle 9.4.1 / compileSdk・targetSdk 36**（v2.16.0〜）。**AGP の上限は Chaquopy が決める**（Chaquopy 17.0 は AGP 7.3〜9.2）ので、上げる前に Chaquopy の対応表を見ること。経緯は `android/design_gui.md` §11.5
